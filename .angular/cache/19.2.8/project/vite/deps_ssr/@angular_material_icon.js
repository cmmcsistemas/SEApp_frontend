...
import {
  BidiModule
} from "./chunk-JNWP5SW4.js";
import {
  DomSanitizer,
  HttpClient
} from "./chunk-LIBVO3XN.js";
import "./chunk-IQ3TZX53.js";
import {
  DOCUMENT,
  isPlatformBrowser
...
var Typeahead = class {
  ...
  _setupKeyHandler(typeAheadInterval) {
    this._letterKeyStream.pipe((0, import_operators5.tap)((letter) => this._pressedLetters.push(letter)), (0, import_operators5.debounceTime)(typeAheadInterval), (0, import_operators5.filter)(() => this._pressedLetters.length > 0), (0, import_operators5.map)(() => this._pressedLetters.join("").toLocaleUpperCase())).subscribe((inputString) => {
...
function coerceObservable(data) {
  if (!(0, import_rxjs7.isObservable)(data)) {
    return (0, import_rxjs7.of)(data);
  }
  return data;
}
...
var TreeKeyManager = class {
  ...
  _items = [];
  _typeahead;
  _typeaheadSubscription = import_rxjs8.Subscription.EMPTY;
  ...
  constructor(items, config) {
    ...
    } else if ((0, import_rxjs8.isObservable)(items)) {
  ...
  change = new import_rxjs8.Subject();
  ...
  _expandCurrentItem() {
    ...
    if (!this._isCurrentItemExpanded()) {
      ...
      coerceObservable(this._activeItem.getChildren()).pipe((0, import_operators6.take)(1)).subscribe((children) => {
  ...
  _expandAllItemsAtCurrentItemLevel() {
    ...
    let itemsToExpand;
    if (!parent) {
      itemsToExpand = (0, import_rxjs8.of)(this._items.filter((item) => item.getParent() === null));
    } else {
      itemsToExpand = coerceObservable(parent.getChildren());
    }
    itemsToExpand.pipe((0, import_operators6.take)(1)).subscribe((items) => {
...
var MatIcon = class _MatIcon {
  ...
  _currentIconFetch = import_rxjs10.Subscription.EMPTY;
  constructor() {
  ...
  _updateSvgIcon(rawName) {
    ...
    if (rawName) {
      ...
      this._currentIconFetch = this._iconRegistry.getNamedSvgIcon(iconName, namespace).pipe((0, import_operators8.take)(1)).subscribe((svg) => this._setSvgElement(svg), (err) => {
